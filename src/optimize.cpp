...
VertexOptimizer::VertexOptimizer ( const QuantizedAttribList& attribs )
    : attribs_ ( attribs ),
      per_vertex_ ( attribs_.size() / 8 ),
      next_unused_index_ ( 0 ) {
    // The cache has an extra slot allocated to simplify the logic in
    // InsertIndexToCache.
    for ( unsigned int i = 0; i < K_CACHE_SIZE + 1; ++i ) {
        cache_[i] = K_UNKNOWN_INDEX;
    }

    // Initialize per-vertex state.
    for ( size_t i = 0; i < per_vertex_.size(); ++i ) {
        VertexData& vertex_data = per_vertex_[i];
        vertex_data.cache_tag = K_CACHE_SIZE;
        vertex_data.output_index = K_MAX_OUTPUT_INDEX;
    }
}

void VertexOptimizer::add_triangles ( const int32_t* indices, size_t length, WebGLMeshList* meshes ) {
    ...
    for ( size_t c = 0; c < per_tri.size(); ++c ) {
        const int best_triangle = find_best_triangle ( indices, per_tri );
        ...
        for ( size_t i = 0; i < 3; ++i ) {
            ...
            vertex_data.remove_face ( best_triangle );

            insert_index_to_cache ( index );
            const int cached_output_index = per_vertex_[index].output_index;
            // Have we seen this index before?
            if ( cached_output_index != K_MAX_OUTPUT_INDEX ) {
        ...
        if ( next_unused_index_ > K_MAX_OUTPUT_INDEX - 3 ) {
            ...
            for ( size_t i = 0; i < per_vertex_.size(); ++i ) {
                per_vertex_[i].output_index = K_MAX_OUTPUT_INDEX;
            }
        }
    }
}
...
void VertexOptimizer::VertexData::update_score() {
    ...
    if ( cache_tag < 3 ) {
        ...
        score = kLastTriScore;
    } else if ( cache_tag < K_CACHE_SIZE ) {
        // Points for being recently used.
        const float kScale = 1.f / ( K_CACHE_SIZE - 3 );
...
void VertexOptimizer::VertexData::remove_face ( int32_t tri ) {
...
int32_t VertexOptimizer::find_best_triangle ( const int32_t* indices, const std::vector<TriangleData>& per_tri ) {
...
void VertexOptimizer::insert_index_to_cache ( int32_t index ) {
    // Find how recently the vertex was used.
    const uint32_t cache_tag = per_vertex_[index].cache_tag;
    ...
    // that case simpler.
    int32_t to_insert = index;
    for ( uint32_t i = 0; i <= cache_tag; ++i ) {
        const int32_t current_index = cache_[i];

        // Update cross references between the entry of the cache and
        // the per-vertex data.
        cache_[i] = to_insert;
        per_vertex_[to_insert].cache_tag = i;
        per_vertex_[to_insert].update_score();

        // No need to continue if we find an empty entry.
        if ( current_index == K_UNKNOWN_INDEX ) {
            break;
        }
